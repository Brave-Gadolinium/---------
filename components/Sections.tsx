...
export function About() {
  ...
  return <section id="about" className="section section--panel"><div className="container"><div className="about-layout"><div className="about-portrait reveal"><Image className="about-portrait-image" src={siteConfig.profileImage} alt={copy.profilePhotoAlt} fill sizes="(max-width: 900px) 100vw, 480px" priority /><div className="about-portrait-shade" aria-hidden="true" /><span>DEV / {siteConfig.name}</span><small>ROBLOX STUDIO · LUAU</small></div><div className="about-copy"><SectionTitle eyebrow={copy.eyebrow} title={copy.title} /><p className="about-lead">{copy.lead}</p><p>{copy.body}</p><div className="about-columns"><div><span>{copy.focus}</span>{focusAreas.map((item, index) => <strong key={index}>{localize(item, language)}</strong>)}</div><div><span>{copy.production}</span>{workExperience.map((item) => <strong key={item.company}>{item.company}</strong>)}</div></div></div></div><div className="experience-block"><div className="experience-heading"><span className="eyebrow">{copy.experience}</span><p>{copy.experienceDescription}</p></div><div className="experience-timeline">{workExperience.map((item, index) => <article className="experience-item reveal" key={item.company}><div className="experience-marker"><span>{String(index + 1).padStart(2, "0")}</span><i /></div><div className="experience-card"><div><h3>{item.company}</h3><span>{localize(item.duration, language)}</span></div><strong>{localize(item.role, language)}</strong><p>{localize(item.description, language)}</p></div></article>)}</div></div></div></section>;
}

export function Contact() {
  const { language } = useLanguage(); const copy = getMessages(language).sections.contact;
  return <section id="contact" className="contact-section" data-copy-allowed><div className="contact-grid" aria-hidden="true" /><div className="container contact-inner"><span className="eyebrow">{copy.eyebrow}</span><h2>{copy.title}<br /><em>{copy.accent}</em></h2><p>{copy.description}</p><div className="contact-links">{siteConfig.contacts.map((contact) => <a key={contact.id} href={contact.href} target={contact.id === "email" ? undefined : "_blank"} rel={contact.id === "email" ? undefined : "noreferrer"}><span><small>{contact.id}</small>{contact.label}</span><ArrowUpRight size={17} /></a>)}</div></div></section>;
}
